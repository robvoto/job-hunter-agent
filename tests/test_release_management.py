...
def _write_runtime_release_files(root: Path) -> None:
    ...
    (root / "scripts" / "ec2" / "deploy-jobhunter.sh").write_text(
...
def test_deploy_rejects_missing_version_argument(tmp_path):
    ...
    result = _run(["bash", "scripts/ec2/deploy-jobhunter.sh"], cwd=repo, env=env, check=False)
...
def test_deploy_rejects_untagged_commit_argument(tmp_path):
    ...
    result = _run(
        ["bash", "scripts/ec2/deploy-jobhunter.sh", head_commit],
        cwd=repo,
        env=env,
        check=False,
    )
...
def test_deploy_checks_out_exact_tagged_commit_not_latest_main(tmp_path):
    ...
    result = _run(
        ["bash", "scripts/ec2/deploy-jobhunter.sh", "v1.5.1"],
        cwd=repo,
        env=env,
        check=False,
    )
...
def test_deploy_rejects_tag_version_mismatch(tmp_path):
    ...
    result = _run(
        ["bash", "scripts/ec2/deploy-jobhunter.sh", "v1.5.2"],
        cwd=repo,
        env=env,
        check=False,
    )
