...
from job_hunter_agent.occupation_taxonomy import OccupationClassification, RESULT_FAR, RESULT_UNCERTAIN, classify_title
...
def test_title_not_target_stops_before_detail_fetch_when_onet_is_far(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        job_review_pipeline,
        "_onet_classify_title",
        lambda title, profile: classify_title(title, project_delivery_profile, db_path=db_path, _index=finance_index),
    )
...
def test_pipeline_logs_job_centric_block_format(caplog, monkeypatch):
    ...
    title_note = next((m for m in messages if "title not in your target roles" in m or "will read description" in m), None)
...
def _patch_review_post_detail_for_work_type_assertions(monkeypatch, expected_work_type):
    def _assert_inferred_work_type(record, profile):
        ...
        assert actual_work_type == expected_work_type, f"expected {expected_work_type}, got {actual_work_type}"
        return True, "OK"
...
def test_full_time_without_contract_keywords_infers_permanent_before_preference_filters(monkeypatch):
