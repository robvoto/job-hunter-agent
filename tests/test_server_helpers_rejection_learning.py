"""Tests for server helpers rejection learning."""

from pytest import MonkeyPatch

from job_hunter_agent import server_helpers, server_review, signal_registry
...
def test_save_requirement_blockers_feedback_adds_blocker_and_suggests_title_followup(
    ...
    monkeypatch.setattr(
        server_review, "persist_review_event", lambda *args, **kwargs: events.append((args, kwargs))
    )
...
def test_save_requirement_blockers_feedback_skips_title_followup_when_kept_history_matches(
    ...
    monkeypatch.setattr(
        server_review, "rebuild_workspace_after_rule_change", lambda reason="": None
    )
...
def test_save_requirement_blockers_feedback_can_apply_title_block_in_same_flow(
    monkeypatch: MonkeyPatch,
):
...
def test_save_requirement_blockers_feedback_suggests_description_block_when_only_rejected_descriptions_match(
    monkeypatch: MonkeyPatch,
):
...
def test_save_requirement_blockers_feedback_can_apply_description_block_in_same_flow(
    monkeypatch: MonkeyPatch,
):
...
def test_save_requirement_blockers_feedback_applies_both_title_and_description_blocks_in_same_flow(
    isolated_db, monkeypatch: MonkeyPatch
):
