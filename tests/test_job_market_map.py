...
@pytest.mark.parametrize(
    ...
    [
        ("year", 110000, 110000, 120000, 0, False, "PREF_SALARY_BELOW_MIN"),
        ("year", 130000, 130000, 120000, 0, True, "OK"),
        ("day", 700, 700, 0, 800, False, "PREF_SALARY_BELOW_MIN"),
        ("day", 900, 900, 0, 800, True, "OK"),
        ("year", 100000, 130000, 120000, 0, True, "OK"),
        ("year", 90000, 110000, 120000, 0, False, "PREF_SALARY_BELOW_MIN"),
        ("year", 130000, 150000, 120000, 0, True, "OK"),
        ("year", 100000, None, 120000, 0, True, "OK"),
        ("year", 130000, None, 120000, 0, True, "OK"),
        ("year", None, 110000, 120000, 0, False, "PREF_SALARY_BELOW_MIN"),
        ("year", None, 130000, 120000, 0, True, "OK"),
    ],
)
def test_jh024_025_jmm_structured_salary_bounds_drive_eligibility_without_bound_field(
    period,
    minimum,
    maximum,
    yearly_floor,
    ...
    item = _item(45)
    item["salary_text"] = "raw salary evidence kept for display"
    item["salary_normalized"] = {
        "state": "known",
        "min_amount": minimum,
        "max_amount": maximum,
    ...
    eligible, reason = preferences.passes_preference_filters(
        ...
        {
            "salary_preferences": {
                "minimum_salary_yearly": yearly_floor,
                "minimum_daily_rate": daily_floor,
            }
        },
    )
    ...
    assert record["salary"] == "raw salary evidence kept for display"
    assert "bound" not in record["market_map_salary_normalized"]


@pytest.mark.parametrize(
    ("state", "period", "currency", "qualifier"),
    [
        ("unknown", "year", "AUD", None),
        ("not_present", None, None, None),
        ("not_applicable", None, None, None),
        ("known", None, "AUD", None),
        ("known", "year", "USD", None),
        ("known", "year", "AUD", "includes_super"),
        ("known", "year", "AUD", "package"),
    ],
)
def test_jh024_025_unknown_or_incomparable_jmm_salary_remains_eligible_without_reparse(
    state, period, currency, qualifier, monkeypatch
):
    from job_hunter_agent import preferences

    item = _item(46)
    item["salary_text"] = "raw salary text"
    item["field_states"] = {
        **item["field_states"],
        "salary": state if state != "unknown" else "known",
    }
    item["salary_normalized"] = {
        "state": state,
        "min_amount": 50000 if state == "known" else None,
        "max_amount": 50000 if state == "known" else None,
        "period": period,
        "currency": currency,
        "qualifier": qualifier,
    }
