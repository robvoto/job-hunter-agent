...
function getSelectedLocationValues() {
  return locationUi.getSelectedLocationValues?.(document.getElementById('locations')) || [];
}

function renderLocationOptions() {
  const container = document.getElementById('locations');
  if (!container || !locationUi.renderLocationCheckboxes) return;
  locationUi.renderLocationCheckboxes(container, {
    selectedValues: loadedProfile?.search_settings?.locations || [],
    maxSelected: loadedGlobalSettings?.limits?.search?.locations_max_selected?.max,
    onChange: () => markDirty(),
  });
}
