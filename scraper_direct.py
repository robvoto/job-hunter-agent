...
DEBUG_JSON_PATH = Path("seek_results.json")
...
def render_html(output_path: str, kept_records: List[dict], run_started_at: datetime, date_range_days: int) -> None:
...
<body>
  <main class="page">
    <section class="hero">
      ...
      <p>Latest run: {safe_html(run_label)}. Search window: roles published within the last {date_range_days} day(s). This file updates each run, while history is tracked separately so repeats are clearly marked.</p>
...
def scrape_seek_jobs_direct(max_pages_cap: int = MAX_PAGES_CAP, headless: bool = False) -> str:
    ...
    base_search_url = set_query_param(SEEK_URL, "daterange", configured_date_range)
        ...
        try:
            ...
            while current_page_num <= configured_max_pages:
                ...
                try:
                    ...
                    print(f"No visible job cards on page {current_page_num}. Stopping. [{type(exc).__name__}]")
                    break
                ...
                if enforce_posted_age_limit and not page_has_fresh_card:
                    print(f"All cards on page {current_page_num} were older than {configured_date_range} day(s). Stopping.")
                    break

                current_page_num += 1

            render_html(OUTPUT_HTML, kept_records, run_started_at, configured_date_range)
