...
def _ignored_capability_suggestion_terms(profile: dict[str, Any]) -> set[str]:
    review_controls = profile.get("review_controls") or {}
    ignored = review_controls.get(_IGNORED_CAPABILITY_SUGGESTIONS_KEY) or []
    return {
        normalized
        for normalized in (_normalize_term(str(skill)) for skill in ignored)
        if normalized
    }
            ...
            suggestions.append(
                {
                    ...
                    "target": "Role targeting and title matching",
                    "recommendation": "Keep role targeting broad unless deeper review volume rises. Tighten title rules only after confirming the broader role family is not useful.",
                    "samples": samples,
                }
            )
            continue
...
def apply_capability_tuning_decisions(
    ...
    profile["review_controls"] = review_controls
    return profile
