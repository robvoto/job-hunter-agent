...
def _load_taxonomy_payload() -> dict[str, Any]:
    if not _INDEX_PATH.is_file():
        raise FileNotFoundError(
            ...
            "Run: uv run python -m job_hunter_agent.onet_taxonomy_refresh --update"
        )
    data = json.loads(_INDEX_PATH.read_text(encoding="utf-8"))
    if not isinstance(data.get("by_normalized_title"), dict):
        raise ValueError(f"O*NET index at {_INDEX_PATH} is missing by_normalized_title")
    return data


@lru_cache(maxsize=1)
def _load_index() -> dict[str, list[dict[str, Any]]]:
    return _load_taxonomy_payload()["by_normalized_title"]


@lru_cache(maxsize=1)
def _load_taxonomy_identity() -> TaxonomyIdentity:
    metadata = _load_taxonomy_payload().get("metadata") or {}
    identity = TaxonomyIdentity(
        taxonomy_version=str(metadata.get("taxonomy_version") or ""),
        database_release=str(metadata.get("database_release") or ""),
        dataset_fingerprint=str(metadata.get("dataset_fingerprint") or ""),
    )
    if not identity.taxonomy_version or not identity.dataset_fingerprint:
        raise ValueError(
            "O*NET index metadata is stale/incomplete; refresh it with "
...
def _derive_target_occupation_codes(
    ...
    a target code only when they resolve to exactly one occupation. Ambiguous job
    ...
    for query in target_occupation_queries:
        ...
        preferred_codes = {
            ...
            and (
                match.get("source") == "occupation_title"
                or bool(match.get("target_query_preferred"))
            )
        }
        if preferred_codes:
            occupation_codes.update(preferred_codes)
            continue
        unique_codes = {
            str(match.get("occupation_code") or "").strip()
            for match in matches
        ...
        if len(unique_codes) == 1:
            occupation_codes.update(unique_codes)
...
def _build_phrase_candidates(index: dict[str, list[dict[str, Any]]]) -> tuple[dict[str, Any], ...]:
    ...
    for normalized_phrase, matches in index.items():
        ...
        job_title_match = next(
            (match for match in matches if match.get("source") in {"job_title", "alternate_title"}),
            None,
        )
        source = "occupation_title" if occupation_title_match is not None else "job_title"
        display_phrase = ""
        if occupation_title_match is not None:
            display_phrase = str(occupation_title_match.get("occupation_title") or "").strip()
        if not display_phrase and job_title_match is not None:
            display_phrase = str(
                job_title_match.get("matched_title")
                or job_title_match.get("occupation_title")
                or ""
            ).strip()
...
def _classify_codes(
    codes: tuple[str, ...],
    target_occupation_codes: set[str],
    *,
    matched_phrase: str | None,
    match_type: str,
) -> OccupationClassification:
    """Classify one or many O*NET candidate codes without discarding ambiguity evidence."""
...
def _classify_embedded_phrase_codes(
    ...
    return _classify_codes(
        tuple(phrase_match["occupation_codes"]),
...
def _representative_match_phrase(matches: list[dict[str, Any]]) -> str | None:
    preferred = next(
        (match for match in matches if match.get("source") == "occupation_title"), None
    )
    ...
    if job_title is not None:
        phrase = str(
            job_title.get("matched_title") or job_title.get("occupation_title") or ""
        ).strip()
        if phrase:
            return phrase
    return None
...
def _cache_lookup(
    normalized_title: str,
    profile_hash: str,
    taxonomy_identity: TaxonomyIdentity,
    db_path: Path | None,
        ...
        row = conn.execute(
            ...
            SELECT result, matched_occupation_code, confidence, matched_phrase, match_type
             ...
             WHERE normalized_title = ?
               ...
               AND database_release = ?
               AND dataset_fingerprint = ?
            """,
            (
                normalized_title,
                profile_hash,
                taxonomy_identity.taxonomy_version,
                taxonomy_identity.database_release,
                taxonomy_identity.dataset_fingerprint,
            ),
        ).fetchone()
    if row is None:
        return None
        ...
        conn.execute(
            ...
            INSERT OR REPLACE INTO occupation_title_cache
                (normalized_title, candidate_profile_hash, taxonomy_version,
                 database_release, dataset_fingerprint, result,
                 matched_occupation_code, confidence, matched_phrase, match_type)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """,
            (
                normalized_title,
...
def classify_title(
    ...
    _taxonomy_identity: TaxonomyIdentity | None = None,
) -> OccupationClassification:
    """Classify a job title as near, far, or uncertain relative to the candidate profile.

    _index/_taxonomy_identity are test seams for isolated unit tests.
    """
    normalized = normalize_title(title)
    index = _index if _index is not None else _load_index()
    taxonomy_identity = _taxonomy_identity or (
        TaxonomyIdentity(TAXONOMY_VERSION, "test", "test-index")
    ...
    cached = _cache_lookup(normalized, profile_hash, taxonomy_identity, db_path)
    ...
    if matches:
        unique_codes = tuple(
            sorted({m["occupation_code"] for m in matches if m.get("occupation_code")})
        )
        matched_phrase = _representative_match_phrase(matches)
        result = _classify_codes(
            unique_codes,
            target_occupation_codes,
            matched_phrase=matched_phrase,
    ...
    _cache_save(normalized, profile_hash, taxonomy_identity, result, db_path)
    _log_classification(
        title,
        normalized,
        profile_target_occupation_queries,
