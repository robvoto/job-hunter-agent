...
def page_login(request: Request, error: str | None = None):  # type: ignore[no-untyped-def]
    if error:
        _human_logger.info("AUTH | login page opened | user=%s | error=%s", _describe_request_user(request), error)
    else:
        _human_logger.info("AUTH | login page opened | user=%s", _describe_request_user(request))
...
def login_google(request: Request):  # type: ignore[no-untyped-def]
    ...
    if not cfg or not cfg.configured:
        _human_logger.info("AUTH | Google sign-in blocked | user=%s | reason=not configured", _describe_request_user(request))
        raise HTTPException(status_code=503, detail="Google sign-in is not configured.")
    state = secrets.token_hex(16)
    auth_url = build_google_auth_url(cfg, state)
    _human_logger.info("AUTH | Google sign-in started | user=%s", _describe_request_user(request))
...
def google_callback(  # type: ignore[no-untyped-def]
    ...
    if error:
        _human_logger.info("AUTH | Google sign-in failed | reason=%s", error)
        return RedirectResponse(f"{LOGIN_PATH}?error={error}", status_code=302)

    cfg = _get_config(request)
    if not cfg or not cfg.configured:
        _human_logger.info("AUTH | Google sign-in failed | reason=not configured")
        raise HTTPException(status_code=503, detail="Google sign-in is not configured.")

    expected_state = request.cookies.get(_OAUTH_STATE_COOKIE)
    if not state or not expected_state or not secrets.compare_digest(state, expected_state):
        _human_logger.info("AUTH | Google sign-in failed | reason=state mismatch")
        return RedirectResponse(f"{LOGIN_PATH}?error=state_mismatch", status_code=302)

    if not code:
        _human_logger.info("AUTH | Google sign-in failed | reason=missing authorization code")
    ...
    try:
        ...
        _human_logger.info("AUTH | Google sign-in failed | reason=token exchange failed")
    ...
    if not email:
        _human_logger.info("AUTH | Google sign-in failed | reason=no email returned")
        return RedirectResponse(f"{LOGIN_PATH}?error=no_email", status_code=302)

    if user_info.get("email_verified") is False:
        _human_logger.info("AUTH | Google sign-in failed | reason=email not verified | email=%s", email)
...
def logout(request: Request):  # type: ignore[no-untyped-def]
    _human_logger.info("AUTH | logged out | user=%s", _describe_request_user(request))
