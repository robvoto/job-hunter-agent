...
def _save_block_similar_feedback_impl(
    ...
    res = _save_feedback_rules_generic(
        ...
        existing_match_fn=lambda r, p: (
            str(r.get("pattern", "")).strip() == build_title_block_rule(p)["pattern"]
        ),
        action="block_title",
    ...
    if added and skipped:
        labels = "', '".join(
            r["reason"].split("'")[1] if "'" in r["reason"] else r["pattern"] for r in added
        )
...
def _save_description_block_feedback_impl(
    ...
    res = _save_feedback_rules_generic(
        ...
        existing_match_fn=lambda r, p: (
            _normalize_description_block_phrase(str(r.get("phrase") or "")) == p
        ),
...
def _collect_blocker_samples(
    matcher_fn: Callable[[dict], bool], audit_rows: list[dict], history: dict[str, dict]
    ...
    def _add_example(row, keys_set, examples_list):
        ...
        job_key = normalize_job_key(
            str(
                row.get(RECORD_JOB_KEY)
                or row.get(RECORD_URL_KEY)
                or row.get(RECORD_TITLE_KEY)
                or ""
            )
        )
...
def _build_block_followups_generic(
    ...
    for blocker in blockers:
        ...
        if (
            ...
            or skip_phrase_fn(phrase)
            or phrase in existing_patterns
        ):
            continue
        ...
        rej_count, kept_count, rej_ex, kept_ex = _collect_blocker_samples(
            matcher_fn, audit_rows, history
        )
    ...
    suggestions.sort(
        key=lambda item: (
            -int(item.get("matched_rejected_count", 0)),
...
def build_description_block_followups(blockers: list[str]) -> list[dict[str, Any]]:
    ...
    def desc_matcher(p: str):
        ...
        return lambda r: (
            p in str(r.get("full_description") or r.get("fit_source_text") or "").strip().lower()
        )
...
def save_requirement_blockers_feedback(
    ...
    applied_titles = (
        list(
            _save_block_similar_feedback_impl(
                normalized, url, title, company, teaser, title_block_phrases
            ).get("block_phrases", [])
        )
    ...
    applied_descs = (
        list(
            _save_description_block_feedback_impl(
                normalized, url, title, company, teaser, description_block_phrases
            ).get("block_phrases", [])
        )
    ...
    if added_blockers or skipped_blockers:
        persist_review_event(
            ...
            extra={
                ...
                "title_block_phrases": applied_titles,
                "description_block_phrases": applied_descs,
            },
        )
    ...
    return {
        ...
        "title_block_suggestions": title_sugg,
        "description_block_suggestions": desc_sugg,
        "applied_title_block_phrases": applied_titles,
        "applied_description_block_phrases": applied_descs,
