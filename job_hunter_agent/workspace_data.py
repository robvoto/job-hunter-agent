...
def build_history_workspace_record(
    ...
    *,
    ...
    return {
        ...
        RECORD_REQUIREMENT_COVERAGE_KEY: snapshot.get(RECORD_REQUIREMENT_COVERAGE_KEY) or [],
        ...
        RECORD_SOURCE_PROVENANCE_KEY: snapshot.get(RECORD_SOURCE_PROVENANCE_KEY) or [],
        ...
        RECORD_IS_LIKED_KEY: bool(entry.get(RECORD_IS_LIKED_KEY, False)),
        RECORD_FIRST_LIKED_AT_KEY: entry.get(RECORD_FIRST_LIKED_AT_KEY),
        RECORD_LAST_LIKED_AT_KEY: entry.get(RECORD_LAST_LIKED_AT_KEY),
        "archived": True,
...
def build_archive_records(
    ...
    *,
    ...
    for job_key, entry in history.items():
        ...
        if (
            not normalized_key
            or normalized_key in current_run_keys
            or normalized_key in blocked_keys
        ):
            continue
...
def build_workspace_record_sets(
    ...
    *,
    ...
    normalize_job_key_fn: Callable[[str], str],
    parse_timestamp_fn: Callable[[Optional[str]], Optional[datetime]],
    build_archive_records_fn: Callable[
        [dict[str, dict], set[str], set[str], set[str], datetime], list[dict]
    ],
...
def load_last_kept_records(
    audit_rows: list[dict], *, deduplicate_across_sources_fn: Callable[[list[dict]], list[dict]]
) -> list[dict]:
    ...
    records = [
        ...
        and str(row.get("run_started_at") or "") == latest_run_started_at
    ]
...
def build_run_stats(
        ...
        if row.get("job_quality_signals"):
            ...
            for signal in row.get("job_quality_signals") or []:
                if isinstance(signal, dict):
                    kind = str(
                        signal.get("kind") or signal.get("label") or "quality_signal"
                    ).strip()
    ...
    rejected_count = sum(
        1 for row in audit_rows if str(row.get("decision") or "").upper() == "REJECT"
    )
    ...
    return {
        ...
        "search_targets": {
            location: sorted(pages) for location, pages in sorted(search_targets.items())
        },
