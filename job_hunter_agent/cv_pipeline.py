...
def cluster_phrases(phrase_items: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for item in phrase_items:
        ...
        role_meta["end_year"] = max(int(role_meta.get("end_year") or 0), int(item.get("end_year") or 0))
    ...
    for phrase, stats in sorted_phrases:
        ...
        if best_index >= 0:
            ...
            for role_key, role_meta in stats["_internal_role_meta"].items():
                ...
                existing_meta["is_recent"] = bool(existing_meta.get("is_recent")) or bool(role_meta.get("is_recent"))
                existing_meta["is_current"] = bool(existing_meta.get("is_current")) or bool(role_meta.get("is_current"))
