...
- **A missing reusable pattern is a signal to add it centrally** (theme CSS + `UI_COMPONENT_MAP.md` entry), not to patch a one-off into the single page that needed it. If the central pattern is unclear or the screen appears to need a genuine page-specific visual exception, stop and ask the human before creating it.
...
- Standard toggle switches must not render a second visible `On` / `Off` / `Enabled` / `Disabled` state label. The slider position already communicates state; extra text that only repeats it is component drift.
