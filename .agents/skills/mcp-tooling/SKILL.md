...
- Job-market collection is separated from Job Hunter policy in `/home/robvoto/projects/job-market-map`. Consume it through the local `/v2` API, never by direct SQLite writes. Neutral job payloads contain market facts only; user activity such as `shown`, `seen`, `reviewed`, `applied`, `rejected`, and `dismissed` lives in the per-user activity ledger. For Rob-aware filtering, use `/v2/users/rob/feed/jobs` or the named consumer feed with `user_key=rob` plus `exclude_activity=...`; do not recreate global job flags in Job Hunter or Job Market Map. Job Hunter remains responsible for fit/scoring/application policy.
