...
- Job-market collection is separated from Job Hunter policy in `/home/robvoto/projects/job-market-map`. Consume neutral market jobs through the local `/v3` API, never by direct SQLite writes. Job Market Map owns market facts and per-consumer feed checkpoints only. JH-305 is the canonical Job Hunter work item for personal activity/outcomes such as `presented_by_agent`, `viewed_by_user`, `applied`, `rejected`, `interview`, and `no_response`, keyed by user + stable market identity (and agent/timestamp where appropriate). Do not recreate a competing activity ledger in Job Market Map. Until JH-305 is live, use the existing authoritative Job Hunter/history sources for personal status.
