...
- `browser_open_tab` opens a brand-new tab instead of reusing whichever tab is currently selected. Use it when the task should not disturb Rob's existing tabs (e.g. starting a fresh application flow), then treat the returned `pageId` as the newly selected tab for subsequent snapshot/click/fill calls.
