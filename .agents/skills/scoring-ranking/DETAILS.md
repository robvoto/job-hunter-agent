...
2. Confirm candidate evidence traces to the **same professional concept** as the requirement — `_has_meaningful_requirement_evidence` accepts it only when: a **single-token** candidate fact (e.g. "Miro", "BigID") is itself named in the requirement wording; or a **multi-word** candidate fact matches as a whole, every substantive token being named in the requirement (identical, or the same word in another inflection — a structural morphology check, `_same_lemma`, with no word list, so "Project delivery" ↔ "Deliver projects" passes); or the whole requirement concept is present in the evidence; or requirement and evidence share **at least two** substantive tokens. One shared modifier word ("AI", "data", "stakeholder", a role/title token) is **not** the same concept — "AI development" does not prove "AI governance", "Stakeholder management" does not prove "Stakeholder facilitation", "Data analysis" does not prove "Data governance".
...
Contract versions: `REQUIREMENT_COVERAGE_CONTRACT_VERSION` = 7, `FIT_REVIEW_CACHE_CONTRACT_VERSION` = 9 (JH-299 + correction follow-up that stopped a single shared modifier token counting as same-concept evidence). Bumping both re-reviews pre-JH-299 coverage/caches fail-closed; no migration path.
...
- **Fail closed on `requirement_kind` (JH-298 correction).** A capability row with a missing, empty, or unrecognised `requirement_kind` is **not** defaulted to `professional_capability` — the structured-output model `_LLMRequirementCoverageItem` carries an **empty** `requirement_kind` default, so a field the model omits reaches normalization as absent and fails closed. It becomes `unclassified`: forced `status = not_assessed`, partitioned by `partition_unclassified_requirement_coverage` into the record's `requirement_coverage_unclassified` field, and — exactly like a behavioural row — non-scoring, non-gap, non-learning, non-actionable until a fresh review classifies it. Scoring credits a row **only** when `requirement_type == "capability"` AND `requirement_kind == "professional_capability"`; `fit_scoring._capability_row_excluded_by_kind` and the `derive_fit_review_grade` loop both skip any other kind — including a missing one — that leaks into `requirement_coverage` (no numerator, no denominator). Contract versions: `REQUIREMENT_COVERAGE_CONTRACT_VERSION` = 7 (`record_schema.py`), `FIT_REVIEW_CACHE_CONTRACT_VERSION` = 9 (`llm_gate.py`) — pre-correction coverage / cache is re-reviewed, not migrated.
