...
O*NET is used as a conservative occupation-family helper. Runtime lookup is local against a generated index built from the current full O*NET Job Titles data. Onboarding-generated `target_occupation_queries` are resolved conservatively so ambiguous real-world aliases cannot silently widen the candidate's target occupation family. Uncertain O*NET results continue to title-LLM/detail review rather than rejecting the job.
