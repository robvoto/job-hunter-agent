...
* `scrapers/seek_runner.py` — SEEK scrape loop, card review dispatch, parallel detail fetch, result collection, and the single owner for visible SEEK sign-in/CAPTCHA waits; persistent SEEK uses its own Playwright profile so login state can survive across searches without sharing a Chromium profile with APSJobs
...
* `source_runner.py` — routes enabled sources; checks the persisted per-source discovery snapshot before running a board search; enabled sources run concurrently via `ThreadPoolExecutor` with isolated mutable state per source when more than one source is active; distinguishes worker completion from source health (`healthy`, `partial_failure`, `full_failure`, `stopped`); commits discovery snapshots only for complete successful collections; step-through keeps the run serial for manual inspection; SEEK human-attention progress is protected from generic slow-source updates so sign-in/verification prompts remain visible until resolved or timed out
...
As of 2026-09-01, explicit duration requirements such as "5+ years as Business Analyst" are resolved by splitting semantics from arithmetic. The fit-review LLM decides which stored `role_experience` family (if any) the stated role/activity belongs to — treating obvious role-family equivalents like Business Analyst / Senior Business Analyst as one family — and returns that exact family name as `matched_role_family` on the experience component. Deterministic code in `experience_requirements.py` then parses the numeric threshold and compares it against that family's accumulated `total_duration_months`. Stated ranges are lower-bounded ("3–5 years" → 36 months). The comparison uses the canonical family total and reports the canonical family name, never the sub-title the LLM was asked about, so crediting a whole family's history cannot be mislabelled as senior-only time. Three outcomes: family resolved and history long enough → coverage stands; family resolved but history short → `experience_duration_gap`, downgrade `supported` → `partially_supported`, and the gap numbers are shown; no family safely resolved (LLM named none, or named one absent from the profile) → `experience_requirement_review_needed`, left unresolved for human review rather than asserting a pass or a fail. The LLM also decomposes the wording into `duration` / `role_or_activity` / `qualifier` fragments; a requirement carrying an unproven `qualifier` is held at `not_shown` even when role history meets the duration, and an incomplete decomposition around a qualifier is likewise held. But when the family resolved and no qualifier is named, a shape-incomplete decomposition (a missing `duration` or `role_or_activity` fragment) no longer forces `not_shown` — the resolved family already carries the LLM's role tie, so the row stays visible on the role-history proof (logged as `experience_requirement_kept_on_role_history`). No duration is ever written back into `candidate_capabilities` — the years a job asks for are compared against captured `role_experience`, never frozen into the profile as a derived capability. Each `role_experience` family also stores per-segment `segments` (`duration_months` + `is_current`, plus `duration_as_of` — the real extraction date — for a still-current role). At match time `experience_requirements.py` and the role-experience matrix in the prompt both use an *effective* family total (`role_experience_duration.py`): completed segments stay at their stored value, and a current segment accrues the whole calendar months elapsed since `duration_as_of`, so a role the candidate still holds stays current without re-onboarding. Completed history remains a snapshot from the last CV/profile refresh. This does not add a new hidden score.
