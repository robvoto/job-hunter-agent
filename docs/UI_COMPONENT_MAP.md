...
| Job requirement badge pair | `workspace_renderer.py` (`render_job_card`) emits `.job-req-importance` and `.job-requirement-status` inside `.job-requirement-badges` | `results-page.css`; both badge types share one geometry/typography rule and differ only by semantic state colours | None | Importance and coverage labels from `workspace_card_labels` in `ui_labels.json` | Job Requirements rows | Do not give importance and coverage badges separate font, padding, height, or alignment rules. The actual matched capability uses the existing `.req-coverage-tag` badge. |
