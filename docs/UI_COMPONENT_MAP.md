...
| Contract-duration inline selector | `#contract_duration_row.contract-duration-row` in `settings-search.html` and `onboarding.html` | `themes.widgets.css` (`.contract-duration-row`, `.contract-duration-row .jh-select`) | `settings-page.js` / `onboarding-page.js` (`updateMinContractMonthState`) | `ui_labels.json` (`min_contract_month_label`, `min_contract_month_help`, `min_contract_month_none_label`, `min_contract_month_options`) | Settings Search basics + Onboarding Search basics | Reveals in normal layout flow under the Contract/FTC chips (no floating overlay). Page CSS must not re-position it absolutely. |
