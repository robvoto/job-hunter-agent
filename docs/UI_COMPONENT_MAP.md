...
| Signals editable review row | `settings-learning.html` / `signal-registry.js` (`.signal-row`) | `settings-page.css` (`.signal-row`, `.signal-value-input`, `.signal-category-wrapper`) | `signal-registry.js` | `signal_registry_labels` in `ui_labels.json` | Settings → Learning → Signals | Three logical columns: wrapping multiline value textarea, category/help plus optional requirement-type selector, and actions. Requirement suggestions may preselect the selector; approval submits the human-visible selection. |
