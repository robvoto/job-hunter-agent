...
| Grouped checkbox list | JS-rendered fieldsets using `.checkbox-list-grid`, `.checkbox-list-group`, `.checkbox-list-options`, `.checkbox-list-option` | `themes.widgets.css` | Page-specific renderer supplies data; Settings uses `settings-page.js` | Owning data/label source | Location selector; reusable for grouped multi-select lists | Page CSS may define only the outer column count |
