...
- `.agents/skills/job-filtering/SKILL.md`: deterministic filters must be explicit, approved, and visible; uncertain signals should not become hidden false-negative gates.
- `.agents/skills/no-hardcoding/SKILL.md`: business judgement, mappings, thresholds, source rules, and reusable knowledge must not be hidden in Python feature code.
